...
from aihive.model import ModelOutput, get_model
...
def test_web_browser_navigation():
    ...
    def is_aihivewebsite(page: str) -> bool:
        return (
            'RootWebArea "aihive" '
            in page
        )

    def is_aihiverepo(page: str) -> bool:
        return (
            'RootWebArea "GitHub -: aihive: A framework for large language model evaluations"'
            in page
        )
    ...
    def check_tool_call_page(tool: str, page: Literal["website", "repo"]) -> None:
        ...
        assert response
        if page == "website":
            assert is_aihivewebsite(response.text)
        elif page == "repo":
            assert is_aihiverepo(response.text)
