...
def pytest_sessionfinish(session, exitstatus):
    if importlib.util.find_spec("aihivepackage"):
        try:
            subprocess.check_call(
                [sys.executable, "-m", "pip", "uninstall", "-y", "aihivepackage"]
            )
            shutil.rmtree("tests/test_package/build")
            shutil.rmtree("tests/test_package/aihivepackage.egg-info")
        except subprocess.CalledProcessError as ex:
            warnings.warn(f"Error occurred uninstalling aihivepackage: {ex}")
