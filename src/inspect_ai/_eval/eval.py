...
from aihive._cli.util import parse_cli_args
from aihive._display.core.active import display
from aihive._util.config import resolve_args
from aihive._util.constants import DEFAULT_LOG_FORMAT
from aihive._util.error import PrerequisiteError
from aihive._util.file import absolute_file_path
from aihive._util.platform import platform_init
from aihive._util.registry import registry_lookup
from aihive.approval._apply import init_tool_approval
from aihive.approval._policy import (
    ApprovalPolicy,
    ApprovalPolicyConfig,
    approval_policies_from_config,
    config_from_approval_policies,
)
from aihive.log import EvalConfig, EvalLog, EvalLogInfo
from aihive.log._file import read_eval_log_async
from aihive.log._recorders import create_recorder_for_format
from aihive.model import (
    GenerateConfig,
    GenerateConfigArgs,
    Model,
)
from aihive.model._model import init_active_model, resolve_models
from aihive.scorer._reducer import reducer_log_names
from aihive.solver._chain import chain
from aihive.solver._solver import Solver, SolverSpec
from aihive.util import SandboxEnvironmentType
from aihive.util._trace import init_trace
    ...
    **kwargs: Unpack[GenerateConfigArgs],
    ...
    Args:
        ...
        model (str | Model | list[str] | list[Model] | None): Model(s) for
            evaluation. If not specified use the value of the aihive_EVAL_MODEL
    ...
    **kwargs: Unpack[GenerateConfigArgs],
    ...
    try:
        ...
        if len(resolved_tasks) == 0:
            log.warning("No aihive tasks were found at the specified paths.")
            return []
        ...
        # resolve recorder
        log_dir = log_dir if log_dir else os.environ.get("aihive_LOG_DIR", "./logs")
...
def eval_init(
    ...
    **kwargs: Unpack[GenerateConfigArgs],
    ...
    if len(model_args) == 0:
        env_model_args = os.environ.get("aihive_EVAL_MODEL_ARGS", None)
        if env_model_args:
