...
aihive_LOG_DIR_VAR = "aihive_LOG_DIR"
...
def init_dotenv() -> None:
    ...
    if dotenv_file:
        # is there an aihive_LOG_DIR currently in the environment? (we will give it preference)
        environment_log_dir = os.environ.get(aihive_LOG_DIR_VAR, None)
        dotenv_log_dir = dotenv_values(dotenv_file).get(aihive_LOG_DIR_VAR, None)
        ...
        if environment_log_dir == dotenv_log_dir or not environment_log_dir:
            ...
            aihivelog_dir = absolute_file_path(environment_log_dir)
        ...
        if aihivelog_dir:
            os.environ[aihive_LOG_DIR_VAR] = aihivelog_dir
