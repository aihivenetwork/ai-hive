...
from aihive._util.constants import SANDBOX
...
def task_project_name(task: str) -> str:
    ...
    return f"aihive-{task}-i{uuid().lower()}"


aihiveproject_pattern = r"^aihive-[a-z\d\-_]*-i[a-z\d]{22}$"


def is_aihiveproject(name: str) -> bool:
    return re.match(aihiveproject_pattern, name) is not None
