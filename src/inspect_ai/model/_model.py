...
from aihive._util.constants import DEFAULT_MAX_CONNECTIONS
from aihive._util.content import ContentText
from aihive._util.hooks import init_hooks, override_api_key, send_telemetry
...
from aihive._util.retry import log_rate_limit_retry
from aihive.tool import Tool, ToolChoice, ToolFunction, ToolInfo
from aihive.tool._tool_def import ToolDef, tool_defs
from aihive.util import concurrency
...
class Model:
    ...
    def verify_model_apis(self) -> None:
        if (
            os.getenv("aihive_DISABLE_MODEL_API", None) is not None
            and ModelName(self).api != "mockllm"
        ):
            raise RuntimeError("Model APIs disabled by aihive_DISABLE_MODEL_API")
    ...
    def _record_model_interaction(
        ...
        from aihive.log._transcript import ModelEvent, transcript
    ...
    **model_args: Any,
    ...
    Args:
       model (str | Model | None): Model specification.
         ...
         then the model referred to by `aihive_EVAL_MODEL`).
    ...
    if model is None:
        ...
        model = os.getenv("aihive_EVAL_MODEL", None)
        if model is not None:
            model = model.split(",")[0]
        else:
            raise ValueError("No model specified (and no aihive_EVAL_MODEL defined)")
...
def resolve_models(
    ...
    if model is None or isinstance(model, str):
        model = model or os.getenv("aihive_EVAL_MODEL", None)
        if model is None:
            raise ValueError("No model specified (and no aihive_EVAL_MODEL defined)")
...
def record_model_usage(model: str, usage: ModelUsage) -> None:
    ...
    from aihive.log._samples import set_active_sample_total_tokens
