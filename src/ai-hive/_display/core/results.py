...
from aihive.log import EvalStats
from aihive.log._log import EvalScore, rich_traceback
    ...
    if samples_completed > 0:
        ...
        if task_can_retry(profile):
            message = (
                ...
                + f"[bold][{theme.light}]aihive eval-retry {log_location}[/{theme.light}][/bold]"
            )
        else:
