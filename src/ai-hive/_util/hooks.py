...
# environment variable (e.g. aihive_TELEMETRY='mypackage.send_telemetry')
...
# Telemetry (aihive_TELEMETRY)
#
# Telemetry can be optionally enabled by setting an aihive_TELEMETRY
...
# API Key Override (aihive_API_KEY_OVERRIDE)
#
# API Key overrides can be optionally enabled by setting an
# aihive_API_KEY_OVERRIDE environment variable which conforms to the
...
def init_hooks() -> None:
    ...
    if len(messages) > 0:
        ...
        print(
            f"[blue][bold]aihive v{version}[/bold][/blue]\n[bright_black]{all_messages}[/bright_black]\n"
        )
