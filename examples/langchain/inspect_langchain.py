...
from aihive.solver import Generate, Solver, TaskState
from aihive.tool import ToolCall, ToolChoice, ToolInfo
...
def langchain_solver(agent: LangChainAgent) -> Solver:
    async def solve(state: TaskState, generate: Generate) -> TaskState:
        # create the aihive model api bridge
        llm = aihiveChatModel()
...
class aihiveChatModel(BaseChatModel):
    ...
    async def _agenerate(
        ...
        **kwargs: dict[str, Any],
        ...
        # generate
        input = [as_aihivemessage(message) for message in messages]
...
def as_aihivemessage(message: BaseMessage) -> ChatMessage:
    if isinstance(message, SystemMessage):
        return ChatMessageSystem(content=as_aihivecontent(message.content))
    elif isinstance(message, HumanMessage):
        return ChatMessageUser(content=as_aihivecontent(message.content))
    elif isinstance(message, AIMessage):
        return ChatMessageAssistant(
            content=as_aihivecontent(message.content),
            tool_calls=(
                [
                    ToolCall(
                        type="function",
