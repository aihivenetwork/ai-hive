...
export class ShowTaskList implements Command {
  ...
  private static readonly id = "aihive.taskOutlineList";
  public readonly id = ShowTaskList.id;
}

export class RunSelectedEvalCommand implements Command {
  constructor(private readonly aihiveEvalMgr_: aihiveEvalManager) { }
  async execute(treeItem: TaskTreeItem): Promise<void> {
    ...
    const evalPromise = this.aihiveEvalMgr_.startEval(
      toAbsolutePath(path),
      task,
      false
    );
    const resumeFocusCommand = "aihive.task-outline-view.focus";
    scheduleReturnFocus(resumeFocusCommand);
    await evalPromise;
  }
  private static readonly id = "aihive.runSelectedTask";
...
export class DebugSelectedEvalCommand implements Command {
  ...
  async execute(treeItem: TaskTreeItem): Promise<void> {
    ...
    const task =
      treeItem.taskPath.type === "task" ? treeItem.taskPath.name : undefined;
    await this.aihiveEvalMgr_.startEval(toAbsolutePath(path), task, true);
  }
  private static readonly id = "aihive.debugSelectedTask";
...
export class EditSelectedTaskCommand implements Command {
  constructor(
    ...
    private aihiveLogviewManager_: aihiveViewManager,
  ...
  async execute() {
    ...
    if (this.tree_.selection.length === 1) {
      ...
      const logViewColumn = this.aihiveLogviewManager_.viewColumn();
  ...
  public static readonly id = "aihive.editSelectedTask";
...
export class CreateTaskCommand implements Command {
  ...
  private static readonly id = "aihive.createTask";
