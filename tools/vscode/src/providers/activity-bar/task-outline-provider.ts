...
export async function activateTaskOutline(
  ...
  const checkaihive = async () => {
    const aihiveAvailable = aihiveVersion() !== null;
    await commands.executeCommand(
      "setContext",
      "aihive.task-outline-view.noaihive",
      !aihiveAvailable
    );
    if (aihiveAvailable) {
  ...
  context.subscriptions.push(
    aihiveManager.onaihiveChanged(async () => {
      await checkaihive();
    })
  );
  await checkaihive();
  ...
  return [
    [
      ...
      new RunSelectedEvalCommand(aihiveEvalMgr),
      new DebugSelectedEvalCommand(aihiveEvalMgr),
      new EditSelectedTaskCommand(tree, aihiveLogviewManager, activeTaskManager),
...
export class TaskOutLineTreeDataProvider
  ...
  public static readonly viewType = "aihive.task-outline-view";
  constructor(
      ...
      this.workspaceMgr.onTasksChanged(
        throttle(
          async (e: TasksChangedEvent) => {
            ...
            await commands.executeCommand(
              "setContext",
              "aihive.task-outline-view.tasksLoaded",
              true
            );
            await commands.executeCommand(
              "setContext",
              "aihive.task-outline-view.noTasks",
              e.tasks?.length === 0
            );
          },
          500,
  ...
  private getNodes(tree: TaskPath[], parent?: TaskTreeItem): TaskTreeItem[] {
    const mode =
      workspace.getConfiguration("aihive").get("taskListView") || "tree";
