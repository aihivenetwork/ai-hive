...
export type NoaihiveCmd = {
  command: "noaihive";
};

export class TaskConfigurationProvider implements WebviewViewProvider {
  public static readonly viewType = "aihive.task-configuration";
  ...
  public async resolveWebviewView(webviewView: WebviewView) {
    ...
    this.disposables_.push(this.aihiveManager_.onaihiveChanged(async () => {
...
const updateSidebarState = async (taskInfo?: DocumentTaskInfo) => {
  await commands.executeCommand(
    "setContext",
    "aihive.task-configuration.task-active",
    taskInfo !== undefined
  );
};
