...
import { kaihiveEnvValues } from "../aihive/aihive-constants";
import { aihiveVersion } from "../../aihive";
import { aihiveChangedEvent, aihiveManager } from "../aihive/aihive-manager";
...
const kaihiveModels: Record<string, string> = {
  "openai": "0.3.8",
  "anthropic": "0.3.8",
  "google": "0.3.8",
  "mistral": "0.3.8",
...
const aihiveModels = () => {
  ...
  return Object.keys(kaihiveModels).filter((key) => {
    const ver = kaihiveModels[key];
    if (descriptor !== null) {
...
export class EnvConfigurationProvider implements WebviewViewProvider {
  public static readonly viewType = "aihive.env-configuration-view";
  ...
  constructor(
    ...
    private readonly aihiveManager_: aihiveManager
  ) { }
  ...
  public resolveWebviewView(webviewView: WebviewView) {
    ...
    this.disposables_.push(this.aihiveManager_.onaihiveChanged(async (e: aihiveChangedEvent) => {
      if (e.available) {
        await initMsg();
      } else {
        await noaihiveMsg();
  ...
  private htmlForWebview(webview: Webview) {
    ...
    const modelOptions = aihiveModels().map((model) => {
...
const envToConfig = (envManager: WorkspaceEnvManager) => {
  ...
  const providerModelStr = env[kaihiveEnvValues.providerModel];
  if (providerModelStr) {
  ...
  const logLevel = env[kaihiveEnvValues.logLevel];
  if (logLevel) {
    config.logLevel = logLevel;
  }

  const logDir = env[kaihiveEnvValues.logDir];
  if (logDir) {
    config.logDir = logDir;
  }

  const maxConnections = env[kaihiveEnvValues.connections];
  if (maxConnections) {
    config.maxConnections = maxConnections;
  }

  const maxRetries = env[kaihiveEnvValues.retries];
  if (maxRetries) {
    config.maxRetries = maxRetries;
  }

  const timeout = env[kaihiveEnvValues.timeout];
  if (timeout) {
    config.timeout = timeout;
  }

  const modelBaseUrl = env[kaihiveEnvValues.modelBaseUrl];
  if (modelBaseUrl) {
...
const configToEnv = (config: EnvConfiguration): Record<string, string> => {
  ...
  if (config.provider && config.model) {
    env[kaihiveEnvValues.providerModel] = `${config.provider}/${config.model}`;
  } else {
    env[kaihiveEnvValues.providerModel] = "";
  }

  env[kaihiveEnvValues.logLevel] = config.logLevel || "";
  env[kaihiveEnvValues.logDir] = config.logDir || "";
  env[kaihiveEnvValues.connections] = config.maxConnections || "";
  env[kaihiveEnvValues.retries] = config.maxRetries || "";
  env[kaihiveEnvValues.timeout] = config.timeout || "";
  env[kaihiveEnvValues.modelBaseUrl] = config.modelBaseUrl || "";
