...
export function activateWorkspaceTaskProvider(aihiveManager: aihiveManager, context: ExtensionContext) {

  // The task manager
  const taskManager = new WorkspaceTaskManager();

  // If the interpreter changes, refresh the tasks
  context.subscriptions.push(aihiveManager.onaihiveChanged(async (e: aihiveChangedEvent) => {
    if (e.available) {
...
export class WorkspaceTaskManager {
  ...
  public async refresh() {
    ...
    try {
      const tasks = await aihiveTaskData(workspace);
      this.setTasks(tasks);
    } catch (err: unknown) {
      console.log("Unable to read aihive task data.");
      console.error(err);
    }
...
async function aihiveTaskData(folder: AbsolutePath) {
