...
import { hasMinimumaihiveVersion, withMinimumaihiveVersion } from "../../aihive/version";
import { kaihiveEvalLogFormatVersion, kaihiveOpenaihiveViewVersion } from "./aihive-constants";
import { aihiveEvalLog, aihiveEvalLogHeaders, aihiveEvalLogs } from "../../aihive/logs";
...
import { aihiveManager } from "./aihive-manager";
import { activeWorkspaceFolder } from "../../core/workspace";


export class aihiveViewServer implements Disposable {
  constructor(context: ExtensionContext, aihiveManager: aihiveManager) {
    // create output channel for debugging
    this.outputChannel_ = window.createOutputChannel("aihive View");

    // shutdown server when aihive version changes (then we'll launch
    // a new instance w/ the correct version)
    context.subscriptions.push(
      aihiveManager.onaihiveChanged(() => {
        this.shutdown();
      })
    );
  }
  ...
  public async evalLogs(log_dir: Uri): Promise<string | undefined> {
    if (this.haveaihiveEvalLogFormat()) {
  ...
  private async ensureRunning(): Promise<void> {
    ...
    // only do this if we have a new enough version of aihive
    if (!this.haveaihiveEvalLogFormat()) {
      return;
    }
    ...
    await this.serverStartupLock_.acquire("server-startup", async () => {
      if (this.serverProcess_ === undefined || this.serverProcess_.exitCode !== null) {
        ...
        return new Promise((resolve, reject) => {
          ...
          // find aihive
          const aihive = aihiveBinPath();
          if (!aihive) {
            throw new Error("aihive view: aihive installation not found");
          }
          ...
          const options: SpawnOptions = {
            ...
            env: {
              "COLUMNS": "150",
              "aihive_VIEW_AUTHORIZATION_TOKEN": this.serverAuthToken_,
            },
            windowsHide: true
          };
          ...
          this.serverProcess_ = spawnProcess(quote(aihive.path), args.map(quote), options, {
            stdout: onOutput,
            stderr: onOutput,
          }, {
            onClose: (code: number) => {
              this.outputChannel_.appendLine(`aihive View exited with code ${code} (pid=${this.serverProcess_?.pid})`);
            },
            onError: (error: Error) => {
              this.outputChannel_.appendLine(`Error starting aihive View ${error.message}`);
              reject(error);
            },
          });
          this.outputChannel_.appendLine(`Starting aihive View on port ${this.serverPort_} (pid=${this.serverProcess_?.pid})`);
        });
  ...
  private haveaihiveEvalLogFormat() {
    return hasMinimumaihiveVersion(kaihiveEvalLogFormatVersion);
  }
...
function evalLogs(log_dir: Uri): Promise<string | undefined> {
  ...
  const response = withMinimumaihiveVersion<string | undefined>(
    kaihiveOpenaihiveViewVersion,
    () => {
      const workspaceRoot = activeWorkspaceFolder().uri;
      const logs = aihiveEvalLogs(activeWorkspacePath(), log_dir);
    ...
    () => {
      ...
      return aihiveEvalLogs(activeWorkspacePath());
    }
  );
...
function evalLog(
  ...
  return Promise.resolve(
    aihiveEvalLog(activeWorkspacePath(), file, headerOnly)
  );
}

function evalLogHeaders(files: string[]) {
  return Promise.resolve(aihiveEvalLogHeaders(activeWorkspacePath(), files));
}
