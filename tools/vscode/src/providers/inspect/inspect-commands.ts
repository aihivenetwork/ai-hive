...
import { kPythonPackageName } from "../../aihive/props";
import { removeFilesSync } from "../../core/file";


export function activateaihiveCommands(
  stateManager: WorkspaceStateManager,
  context: ExtensionContext) {
  const aihiveCommands = new aihiveCommandDispatcher(stateManager);
  context.subscriptions.push(aihiveCommands);
}


export class aihiveCommandDispatcher implements Disposable {

  constructor(stateManager: WorkspaceStateManager) {
    // init commands dir and remove any existing commands
    this.commandsDir_ = aihiveCommandsDir(stateManager);
...
function aihiveCommandsDir(stateManager: WorkspaceStateManager): string {
