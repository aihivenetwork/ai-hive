...
import { aihiveEvalCommands } from "./aihive-eval-commands";
...
export async function activateEvalManager(
...
): Promise<[Command[], aihiveEvalManager]> {
  // Activate the manager
  const aihiveEvalMgr = new aihiveEvalManager(stateManager);
  ...
  env.delete("aihive_WORKSPACE_ID");
  env.append("aihive_WORKSPACE_ID", workspaceId);

  return [aihiveEvalCommands(aihiveEvalMgr), aihiveEvalMgr];
}

export class aihiveEvalManager {
  ...
  public async startEval(file: AbsolutePath, task?: string, debug = false) {
    // if we don't have aihive bail and let the user know
    if (!aihiveVersion()) {
      await window.showWarningMessage(
        `Unable to ${debug ? "Debug" : "Run"
        } Eval (aihive Package Not Installed)`,
        {
          modal: true,
          detail: "pip install --upgrade aihive-ai",
        }
      );
      return;
    }
    ...
    if (
      debug === true &&
      workspace.getConfiguration("aihive").get("debugSingleSample")
    ) {
      args.push(...["--limit", "1"]);
    } else if (limit) {
    ...
    const useSubdirectoryEnvironments = workspace.getConfiguration("aihive").get("useSubdirectoryEnvironments");
    ...
    if (debug) {
      ...
      const env = {
        aihive_WORKSPACE_ID: this.stateManager_.getWorkspaceInstance(),
      };

      await runDebugger(
        aihiveBinPath()?.path || "aihive",
        args,
        workspaceDir.path,
        debugPort,
        env,
