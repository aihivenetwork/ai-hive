...
import {
  aihiveWebview,
  aihiveWebviewManager,
} from "../../components/webview";
import { aihiveViewPath } from "../../aihive/props";
import {
  aihiveChangedEvent,
  aihiveManager,
} from "../aihive/aihive-manager";
...
const kLogViewId = "aihive.logview";


export class aihiveViewManager {
  constructor(
    private readonly context_: ExtensionContext,
    private readonly webViewManager_: aihiveViewWebviewManager,
    private readonly envMgr_: WorkspaceEnvManager,
    logsWatcher: aihiveLogsWatcher
  ) {

    this.context_.subscriptions.push(logsWatcher.onaihiveLogCreated(async (e) => {
...
export class aihiveViewWebviewManager extends aihiveWebviewManager<
  aihiveViewWebview,
  LogviewState
> {
  constructor(
    ...
    context.subscriptions.push(
      aihiveManager.onaihiveChanged((e: aihiveChangedEvent) => {
  ...
  protected override getWorkspaceState(): LogviewState | undefined {
    const data: Record<string, string> = this.context_.workspaceState.get(this.kaihiveViewState, {});
    if (data) {
      return {
  ...
  protected setWorkspaceState(state: LogviewState) {
    void this.context_.workspaceState.update(this.kaihiveViewState, {
  ...
  private kaihiveViewState = 'aihiveViewState';
...
class aihiveViewWebview extends aihiveWebview<LogviewState> {
  ...
  public setManager(manager: aihiveViewWebviewManager) {
    if (this._manager !== manager) {
      this._manager = manager;
    }
  }
  _manager: aihiveViewWebviewManager | undefined;
