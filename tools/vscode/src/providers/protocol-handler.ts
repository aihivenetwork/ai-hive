...
export function activateProtocolHandler(context: ExtensionContext) {
  const protocolHandler = new aihiveProtocolHandler();
  context.subscriptions.push(window.registerUriHandler(protocolHandler));
}

export class aihiveProtocolHandler implements UriHandler {
    ...
    switch (command) {
      ...
      case "open": {
        ...
        if (logFile) {
          ...
          await commands.executeCommand('aihive.openLogViewer', logUri);
        }
      }
    }
  }
